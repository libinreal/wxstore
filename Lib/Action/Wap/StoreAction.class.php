...
class StoreAction extends WapAction{
    ...
    public function bdproducts()
	{
		...
		if (isset($_GET['keyword']) && $_GET['keyword']){
            $where['p.name|p.keyword'] = array('like', "%".$_GET['keyword']."%");
            $this->assign('isSearch', 1);
            $this->assign('keyword', $_GET['keyword']);
		}
		$count = $this->product_model->alias('p')->where($where)->count();
		$this->assign('count', $count);
		//排序方式
		$method = isset($_GET['method']) && ($_GET['method']=='DESC' || $_GET['method']=='ASC') ? $_GET['method'] : 'ASC';
		$orders = array('p.time', 'p.discount', 'p.price', 'p.salecount');
		$order = isset($_GET['order']) && in_array($_GET['order'], $orders) ? $_GET['order'] : 'p.sort';
		$this->assign('order', $order);
		$this->assign('method', $method);

		if ($order == 'p.salecount') $order = '(p.salecount+p.fakemembercount)';
		$products = $this->product_model->alias('p')->join('tp_product_cat as c ON p.catid = c.id')->field('p.*, c.name as cat_name')->where($where)->order($order . ' ' . $method)->/*limit('0, 10')->*/select();
		...
		$recommands = local_curl(array(), C('BD_RECOMMAND_URL') . $userInfo['id'] , 0);//var_dump($recommands);exit;
/*
		if( empty( $recommands ) )*/
		...
		if( !empty($recommands) ){
			...
			$item_url_pre = "://" . $_SERVER["HTTP_HOST"] . '/';
			...
			unset( $rv );
			$goods_logos = M('product')->where( array( 'id' => array( 'in', $good_ids ) ) )->getField('id,logourl,name,price');

			foreach ($recommands as &$rv) {
				$old_pic = $goods_logos[ $rv['goods_id'] ]['logourl'];
				...
				if( !is_file( $new_path ) ){
					...
					setWater( $goods_logos[ $rv['goods_id'] ]['logourl'] , './Public/Wap_store_tuijian1.png' , '荐', '0xB0,0xC4,0xDE', '3', './Tpl/Wap/default/common/black/fontawesome-webfont.ttf', 'img' , $new_path);
				}
				$rv['logourl'] = $new_pic;
				$rv['name'] = $goods_logos[ $rv['goods_id'] ]['name'];
				$rv['price'] = $goods_logos[ $rv['goods_id'] ]['price'];
				$rv['id'] = $rv['goods_id'];
		...
		$recommands = array_slice($recommands, 0, 4);
	...
	public function bdproduct()
	{
		...
		$full_page_url = cur_page_url();
		$usertag['itemid'] = preg_replace('/^(http:\/\/|https:\/\/)([^\/]+\/)(index\.php\/)?(.+)$/', "$4", $full_page_url);//$this->_get('id', 'intval', 0);
		$usertag['action'] = 1;
	...
	public function bdmark()
	{
		...
		$userInfo = cookies('userinfo');
		$good_id = $this->_get('id', 'intval');
		$goods_logos = M('product')->where( array( 'id' => $good_id ) )->getField('id,logourl,name,price');

		$old_pic = $goods_logos[ $good_id ]['logourl'];
		$new_pic = preg_replace('/^(.+)\.(jpeg|jpg|png|gif)/', '\1_duigou.\2', $old_pic);
		preg_match('/https?:\/\/[^\/]*((\/.*)\.(jpeg|jpg|png|gif))/', $old_pic ,$mathes_path);

		// echo APP_PATH;exit;
		$new_path = DOCUMENT_ROOT . $mathes_path[2] . '_duigou.' . $mathes_path[3];
		if( !is_file( $new_path ) ){
			setWater( $goods_logos[ $good_id ]['logourl'] , './Public/Wap_store_duigou.png' , '√', '0xB0,0xC4,0xDE', '3', './Tpl/Wap/default/common/black/fontawesome-webfont.ttf', 'img' , $new_path);
		}


		$full_page_url = 'Wap/Store/bdproduct/id/'.$good_id;
