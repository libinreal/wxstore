...
<div><a href="{:U('Store/myinfo')}" class="uc">用户中心</a></div>
<div><a href="{:U('Store/cart')}" class="cart">购物车<i class="cart_com"><?php if($totalProductCount != 0): echo ($totalProductCount); endif; ?></i></a></div>
</div>

<ul class="sub-menu-list">
<li><a href="{:U('Store/select',array('token' => $_GET['token'], 'wecha_id' => $wecha_id, 'twid' => $twid)}">浏览店铺</a></li>
<li><a href="{:U('Store/cats',array('token' => $_GET['token'], 'catid' => $hostlist['id'], 'wecha_id' => $wecha_id, 'cid' => $cid, 'twid' => $twid, 'cid' => $cid)}">商城首页</a></li>
<?php if(is_array($cats)): $i = 0; $__LIST__ = $cats;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$hostlist): $mod = ($i % 2 );++$i; if($hostlist['isfinal'] == 1): ?><li><a href="{:U('Store/products',array('token' => $_GET['token'], 'catid' => $hostlist['id'], 'wecha_id' => $wecha_id, 'twid' => $twid, 'cid' => $cid)}"><?php echo ($hostlist["name"]); ?></a></li>
<?php else: ?>
<li><a href="{:U('Store/cats',array('token' => $_GET['token'], 'cid' => $hostlist['cid'], 'parentid' => $hostlist['id'], 'wecha_id' => $wecha_id, 'twid' => $twid, 'cid' => $cid)}"><?php echo ($hostlist["name"]); ?></a></li><?php endif; endforeach; endif; else: echo "" ;endif; ?>
</ul>
...
<?php if(is_array($products)): $i = 0; $__LIST__ = $products;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$hostlist): $mod = ($i % 2 );++$i;?><li>
		<span class="pic">
			<a href="{pigcms::U('Store/product',array('token' => $_GET['token'], 'id' => $hostlist['id'], 'twid' => $twid))}">
			<img src="{pigcms:$hostlist.logourl}" data-original="{pigcms:$hostlist.logourl}">
		</a>
		</span>
		<span class="con">
			<a class="t" href="{pigcms::U('Store/product',array('token' => $_GET['token'], 'id' => $hostlist['id'], 'twid' => $twid))}">{pigcms:$hostlist.name}</a>
			<b>￥{pigcms:$hostlist.price}</b><del>￥{pigcms:$hostlist.oprice}</del>
			<span style="color:#999;font-size:8px;margin-top:15px;">{pigcms:$hostlist['salecount'] + $hostlist['fakemembercount']}人付款</span>
		</span>
...
$(function() {
	...
	//点击排序
	var base_url = '{pigcms::U('Store/products',array('token'=>$_GET['token'],'catid'=>$thisCat['id'], 'twid' => $twid))}';
	var b_url = '<?php if($isSearch != 1): ?>{pigcms::U('Store/ajaxProducts',array('token'=>$_GET['token'],'catid'=>$thisCat['id'], 'twid' => $twid))}<?php else: ?>{pigcms::U('Store/ajaxProducts',array('token'=>$_GET['token'],'keyword'=>$_GET['keyword'], 'twid' => $twid))}<?php endif; ?>'
		method = 'DESC',
	...
	var com_link = '{pigcms::U('Store/product',array('token'=>$_GET['token'], 'twid' => $twid))}';
...
window.shareData = {
            ...
            "timeLineLink": "<?php echo ($f_siteUrl); ?>{pigcms::U('Store/products',array('token' => $_GET['token'], 'catid' => $thisCat['id'], 'twid' => $mytwid, 'cid' => $cid))}",
            "sendFriendLink": "<?php echo ($f_siteUrl); ?>{pigcms::U('Store/products',array('token' => $_GET['token'], 'catid' => $thisCat['id'], 'twid' => $mytwid, 'cid' => $cid))}",
            "weiboLink": "<?php echo ($f_siteUrl); ?>{pigcms::U('Store/products',array('token' => $_GET['token'], 'catid' => $thisCat['id'], 'twid' => $mytwid, 'cid' => $cid))}",
